...
grep -Fq 'root.runAction("setup", app)' "$QML"
grep -Fq 'root.runAction("repair", app)' "$QML"
grep -Fq 'root.runAction("install", app)' "$QML"
grep -Fq 'root.runAction(root.appRunning(app.id) ? "uninstall-close" : "uninstall", app)' "$QML"
grep -Fq 'root.runAction("launch", app)' "$QML"
grep -Fq 'root.refreshCatalog()' "$QML"
...
grep -Fq 'disabled: !root.interactive' "$ACTION"
grep -Fq 'property bool interactive: true' "$ICON"
grep -Fq 'disabled: !root.interactive' "$ICON"
